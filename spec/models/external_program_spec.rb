  ...
  describe '#expired_episodes' do
    ...
    context 'has days_to_expiry timestamp' do
      it "only returns expired episodes" do
        expect(program.expired_episodes.any?{|e| e.created_at < 3.days.ago}).to eq(true)
        expect(program.expired_episodes.any?{|e| e.created_at > 3.days.ago}).to eq(false)
      end
    end
